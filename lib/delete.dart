...
class _DismissibleState extends State<Dismissible> with TickerProviderStateMixin, AutomaticKeepAliveClientMixin { // ignore: MIXIN_INFERENCE_INCONSISTENT_MATCHING_CLASSES
	...
	_dragUnderway = true;
	print('-- start  ${_moveController.value}  ${_overallDragAxisExtent}  ${_dragExtent}  ${_dragExtent.sign}');
	if (_moveController.isAnimating) {
	    _dragExtent = _moveController.value * _overallDragAxisExtent * _dragExtent.sign;
	    _moveController.stop();
	} else {
	    if(0 != _moveController.value) {
	        //TODO add by
//	        _moveController.forward();
	...
	switch (widget.direction) {
		...
		_dragExtent += delta;
		print('-- update  delta: $delta  dragExtent: ${_dragExtent}');
		break;
...
//	        print('--- x: ${details.delta.dx.toStringAsFixed(2)}  y: ${details.delta.dy.toStringAsFixed(2)}  distance: ${details.delta.distance.toStringAsFixed(2)}  direction: ${details.delta.direction}');
//		print('-- update deltapri: ${delta.toStringAsFixed(2)}  dragExtent: ${_dragExtent.toStringAsFixed(2)}  ${Directionality.of(context)}');
...
//	print('----- isAnimation: ${_moveController.isAnimating}');
	if (!_moveController.isAnimating) {
	...
	print('&&&   moveAnimation: $end  ${widget.crossAxisEndOffset}');
	...
	print('--- end  ${details.velocity.pixelsPerSecond.dx}  ${details.velocity.pixelsPerSecond.dy}');
	switch (_describeFlingGesture(details.velocity)) {
	    ...
	    case _FlingGestureKind.none:
	        print('---  end none  ${_moveController.value}  ${widget.dismissThresholds[_dismissDirection]}  $_kDismissThreshold');
...
//		    }
		 	...
		 	if(_dragExtent.abs() > menuWidth * rate) {
			    _moveController.forward();
			}else{
			    _moveController.reverse();
			}
	...
	print(' *** resize changed  drag: ${_dragExtent.toStringAsFixed(2)}  resize: ${_resizeController.value.toStringAsFixed(2)}  move: ${_moveController.value.toStringAsFixed(2)}' );
	if(_resizeController.value > 0.5) {
	    _resizeController.stop(canceled: true);
...
//	}
	...
	print('------ ${_dragExtent.abs()}  $menuWidth');
//	if (_dragExtent.abs() >= menuWidth) {
//	    print('------ stop');
//	    _moveController.stop();
//	}
...
class Fuck extends StatefulWidget {
    ...
    @override
    _FuckState createState() => new _FuckState();
}
...
class _FuckState extends State<Fuck> {
  ...
  void initState() {
    ...
    Widget body() {
	var shit = new LayoutBuilder(builder: (context, co) {
	    print(' ${co.minHeight}  ${co.maxHeight}  ${co.biggest}');
	    return widget.widget;
	});
...
//		widget.menu
    ...
    Widget item() {
	    ...
	    child: widget.widget,
	    rect: new Rect.fromLTWH(40.0, .0, 300.0, 30.0),
	);
    }
}
